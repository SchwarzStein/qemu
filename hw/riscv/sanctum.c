...
static const MemMapEntry  sanctum_memmap[] = {
    ...
    [SANCTUM_UART0]        = {  0x3001000,      0x100 },
    [SANCTUM_PLATFORM_BUS] = {  0x4000000,  0x2000000 },
    ...
    [SANCTUM_PCIE_MMIO]    = { 0x40000000,  0x40000000 },
...
{
    /*
     * Create a single flash device. We use the same parameters as the 
     * flash devices on the ARM virt board.
     */
...
static void  virt_flash_map(RISCVVirtState *s,
                            MemoryRegion *sysmem)
{
    hwaddr flashsize = sanctum_memmap[SANCTUM_VIRT_FLASH].size / 2; 
    hwaddr flashbase = sanctum_memmap[SANCTUM_VIRT_FLASH].base; 
                                ...
                                uint32_t irqchip_handle)
{
     ...
     * possible slot) seeing the interrupt-map_mask will allow the table 
    ...
    for (dev = 0; dev < GPEX_NUM_IRQS; dev++) {
        ...
        for (pin = 0; pin < GPEX_NUM_IRQS; pin++) {
            ...
            /* Fill PIC Interrupt cells*/
            irq_map[i] = cpu_to_be32(pin + 1);
            i += FDT_PLIC_INT_CELLS;

            /*Fill interrupt controller phandle and cells*/
            irq_map[i++] = cpu_to_be32(irqchip_handle);
            irq_map[i++] = cpu_to_be32(irq_nr);
            if (s->aia_type != VIRT_AIA_TYPE_NONE) {
                irq_map[i++] = cpu_to_be32(0x04);
            }
                                    ...
                                    bool is_32_bit, uint32_t *intc_phandles)
{
    ...
    for (cpu = s->soc[socket].num_harts - 1; cpu >= 0; cpu--) {
        ...
        g_free(name);
        qemu_fdt_setprop_string(mc->fdt, cpu_name, "compatible", "riscv,ucbbar,spike-bare-dev");//changed
        qemu_fdt_setprop_string(mc->fdt, cpu_name, "status", "okay");
        qemu_fdt_setprop_cell(mc->fdt, cpu_name, "reg", cpu);
        ...
        if (riscv_feature(&s->soc[socket].harts[cpu].env ,RISCV_FEATURE_AIA)) {
            static const char *const compat[2] = {
                "riscv,cpu-intc-aia", "risv,cpu-intc"
            };
                                     ...
                                     const MemMapEntry *memmap, int socket)
{
    ...
    addr = sanctum_memmap[SANCTUM_DRAM].base  + riscv_socket_mem_offset(mc, socket);
                                    ...
                                    uint32_t *intc_phandles)
{
    ...
    clint_addr = memmap[SANCTUM_CLINT].base  + (memmap[SANCTUM_CLINT].size *socket);
    ...
    qemu_fdt_setprop_cells(mc->fdt, clint_name, "reg",
            0x0, clint_addr, 0x0, memmap[SANCTUM_CLINT].size);
                                    ...
                                    uint32_t *plic_phandles)
{
    ...
    for (cpu = 0; cpu < s->soc[socket].num_harts; cpu++) {
        if(kvm_enabled()) {
            ...
            plic_cells[cpu * 2 + 0] = cpu_to_be32(intc_phandles[cpu]);
            plic_cells[cpu * 2 + 1] = cpu_to_be32(IRQ_M_EXT);
            plic_cells[cpu * 2 + 2] = cpu_to_be32(intc_phandles[cpu]);
            plic_cells[cpu * 2 + 3] = cpu_to_be32(IRQ_S_EXT);   
        }
    }

    plic_phandles[socket] = (*phandle)++;
    plic_addr = memmap[SANCTUM_PLIC].base + (memmap[SANCTUM_PLIC].size * socket);
    ...
    qemu_fdt_setprop_cells(mc->fdt, plic_name, "reg",
        0x0, plic_addr, 0x0, memmap[SANCTUM_PLIC].size);
    qemu_fdt_setprop_cell(mc->fdt, plic_name, "riscv,ndev", VIRTIO_NDEV); //changed
    ...
    if (!socket) {
        platform_bus_add_all_fdt_nodes(mc->fdt, plic_name,
                                       memmap[SANCTUM_PLATFORM_BUS].base,
                                       memmap[SANCTUM_PLATFORM_BUS].size,
                                       VIRT_PLATFORM_BUS_IRQ);//changed
    }
                             ...
                             uint32_t *msi_m_phandle, uint32_t *msi_s_phandle)
{
    ...
    *msi_s_phandle = (*phandle)++;
    ...
    for (socket = 0; socket < riscv_socket_count(mc); socket++) {
        imsic_addr = memmap[SANCTUM_IMSIC_M].base +
    ...
    imsic_name = g_strdup_printf("/soc/imsics@%lx",
        (unsigned long)memmap[SANCTUM_IMSIC_M].base);
    ...
    imsic_max_hart_per_socket = 0;
    for (socket = 0; socket < riscv_socket_count(mc); socket++) {
        imsic_addr = memmap[SANCTUM_IMSIC_S].base +
                     socket * VIRT_IMSIC_GROUP_MAX_SIZE;
    ...
    imsic_name = g_strdup_printf("/soc/imsics@%lx",
        (unsigned long)memmap[SANCTUM_IMSIC_S].base);
    qemu_fdt_add_subnode(mc->fdt, imsic_name);
    qemu_fdt_setprop_string(mc->fdt, imsic_name, "compatible",
        "riscv,imsics");
    qemu_fdt_setprop_cell(mc->fdt, imsic_name, "#interrupt-cells",
                                    ...
                                    uint32_t *aplic_phandles)
{
    ...
    aplic_addr = memmap[SANCTUM_APLIC_M].base +
                 (memmap[SANCTUM_APLIC_M].size * socket);
    ...
    qemu_fdt_setprop_cells(mc->fdt, aplic_name, "reg",
        0x0, aplic_addr, 0x0, memmap[SANCTUM_APLIC_M].size);
    ...
    aplic_addr = memmap[SANCTUM_APLIC_S].base +
                 (memmap[SANCTUM_APLIC_S].size * socket);
    aplic_name = g_strdup_printf("/soc/aplic@%lx", aplic_addr);
    qemu_fdt_add_subnode(mc->fdt, aplic_name);
    qemu_fdt_setprop_string(mc->fdt, aplic_name, "compatible", "riscv,aplic");
    qemu_fdt_setprop_cell(mc->fdt, aplic_name,
    ...
    qemu_fdt_setprop_cells(mc->fdt, aplic_name, "reg",
        0x0, aplic_addr, 0x0, memmap[SANCTUM_APLIC_S].size);
    qemu_fdt_setprop_cell(mc->fdt, aplic_name, "riscv,num-sources",
        VIRT_IRQCHIP_NUM_SOURCES);
    riscv_socket_fdt_write_id(mc, mc->fdt, aplic_name, socket);
                              ...
                              uint32_t irq_virtio_phandle)
{
    ...
    for (i = 0; i < VIRTIO_COUNT; i++) { //changed
                            ...
                            uint32_t msi_pcie_phandle)
{
    ...
    name = g_strdup_printf("/soc/pci@%lx",
        (long) memmap[SANCTUM_PCIE_ECAM].base);
    qemu_fdt_add_subnode(mc->fdt, name);
    ...
    qemu_fdt_setprop_cells(mc->fdt, name, "bus-range", 0,
        memmap[SANCTUM_PCIE_ECAM].size / PCIE_MMCFG_SIZE_MIN - 1);
    qemu_fdt_setprop(mc->fdt, name, "dma-coherent", NULL, 0);
    if (s->aia_type == VIRT_AIA_TYPE_APLIC_IMSIC) {
        qemu_fdt_setprop_cell(mc->fdt, name, "msi-parent", msi_pcie_phandle);
    }
    qemu_fdt_setprop_cells(mc->fdt, name, "reg", 0,
        memmap[SANCTUM_PCIE_ECAM].base, 0, memmap[SANCTUM_PCIE_ECAM].size);
    qemu_fdt_setprop_sized_cells(mc->fdt, name, "ranges",
        1, FDT_PCI_RANGE_IOPORT, 2, 0,
        2, memmap[SANCTUM_PCIE_PIO].base, 2, memmap[SANCTUM_PCIE_PIO].size,
        1, FDT_PCI_RANGE_MMIO,
        2, memmap[SANCTUM_PCIE_MMIO].base,
        2, memmap[SANCTUM_PCIE_MMIO].base, 2, memmap[SANCTUM_PCIE_MMIO].size,
                            ...
                            uint32_t irq_mmio_phandle)
{
    ...
    name = g_strdup_printf("/soc/uart@%lx", (long)memmap[SANCTUM_UART0].base);
    qemu_fdt_add_subnode(mc->fdt, name);
    ...
    if (s->aia_type == VIRT_AIA_TYPE_NONE) {
        qemu_fdt_setprop_cell(mc->fdt, name, "interrupts",  UART0_IRQ); //changed 
    } else {
        qemu_fdt_setprop_cells(mc->fdt, name, "interrupts", UART0_IRQ, 0x4); //changed 
    }
                           ...
                           uint32_t irq_mmio_phandle)
{
    ...
    name = g_strdup_printf("/soc/rtc@%lx", (long)memmap[SANCTUM_RTC].base);
    qemu_fdt_add_subnode(mc->fdt, name);
    qemu_fdt_setprop_string(mc->fdt, name, "compatible",
        "google,goldfish-rtc");
    qemu_fdt_setprop_cells(mc->fdt, name, "reg",
        0x0, memmap[SANCTUM_RTC].base, 0x0, memmap[SANCTUM_RTC].size);
    qemu_fdt_setprop_cell(mc->fdt, name, "interrupt-parent",
        irq_mmio_phandle);
    if (s->aia_type == VIRT_AIA_TYPE_NONE) {
        qemu_fdt_setprop_cell(mc->fdt, name, "interrupts", RTC_IRQ);//changed
    } else {
        qemu_fdt_setprop_cells(mc->fdt, name, "interrupts", RTC_IRQ, 0x4);//changed
    }
    g_free(name);
}
...
static void create_fdt_fw_cfg(RISCVVirtState *s, const MemMapEntry *memmap)
{
    char *nodename;
    MachineState *mc = MACHINE(s);
    hwaddr base = memmap[SANCTUM_FW_CFG].base;
    hwaddr size = memmap[SANCTUM_FW_CFG].size;
                       ...
                       uint64_t mem_size, const char *cmdline, bool is_32_bit)
{
    ...
    qemu_fdt_setprop_string(mc->fdt, "/", "model", "ucbbar,sanctum-bare,qemu");
    qemu_fdt_setprop_string(mc->fdt, "/", "compatible", "ucbbar,spike-bare-dev");
    qemu_fdt_setprop_cell(mc->fdt, "/", "#size-cells", 0x2);
    qemu_fdt_setprop_cell(mc->fdt, "/", "#address-cells", 0x2);

    qemu_fdt_add_subnode(mc->fdt, "/htif");
    qemu_fdt_setprop_string(mc->fdt, "/htif", "compatible", "ucb,htif0");
...
{
    hwaddr base = sanctum_memmap[SANCTUM_FW_CFG].base;
    FWCfgState *fw_cfg;
                                     ...
                                     int base_hartid, int hart_count)
{
    ...
    ret = sifive_plic_create(
            memmap[SANCTUM_PLIC].base + socket * memmap[SANCTUM_PLIC].size,
                                    ...
                                    int base_hartid, int hart_count)
{
    ...
    if (msimode) {
        ...
        addr = memmap[SANCTUM_IMSIC_M].base + socket * VIRT_IMSIC_GROUP_MAX_SIZE;
        ...
        addr = memmap[SANCTUM_IMSIC_S].base + socket * VIRT_IMSIC_GROUP_MAX_SIZE;
        for (i = 0; i < hart_count; i++) {
    ...
    aplic_m = riscv_aplic_create(
        memmap[SANCTUM_APLIC_M].base + socket * memmap[SANCTUM_APLIC_M].size,
        memmap[SANCTUM_APLIC_M].size,
    ...
    if (aplic_m) {
        ...
        riscv_aplic_create(
            memmap[SANCTUM_APLIC_S].base + socket * memmap[SANCTUM_APLIC_S].size,
            memmap[SANCTUM_APLIC_S].size,
            (msimode) ? 0 : base_hartid,
            (msimode) ? 0 : hart_count,
            VIRT_IRQCHIP_NUM_SOURCES,
            VIRT_IRQCHIP_NUM_PRIO_BITS,
...
static void sanctum_virt_machine_done(Notifier *notifier, void *data)
{
...
static void sanctum_virt_machine_init(MachineState *machine)
{
    ...
    assert(PGSHIFT == 12);
    assert(machine->ram_size == 0x80000000);
    /* Due to hacks on hacks on hack emulator is only defined for a machine with 
     *  2GB DRAM and 64 "regions" for enclave isolation.
     */
        ...
        if (!kvm_enabled()) {
            if (s->have_aclint) {
                if (s->aia_type == VIRT_AIA_TYPE_APLIC_IMSIC) {
                    ...
                    riscv_aclint_mtimer_create(memmap[SANCTUM_CLINT].base +
                    ...
                    riscv_aclint_swi_create(memmap[SANCTUM_CLINT].base +
                            i * memmap[SANCTUM_CLINT].size,
                    ...
                    riscv_aclint_mtimer_create(memmap[SANCTUM_CLINT].base +
                        ...
                        RISCV_ACLINT_DEFAULT_MTIME,
                        RISCV_ACLINT_DEFAULT_TIMEBASE_FREQ, true);
                    riscv_aclint_swi_create(memmap[SANCTUM_ACLINT_SSWI].base +
                            i * memmap[SANCTUM_ACLINT_SSWI].size,
                        base_hartid, hart_count, true);
                }
            } else {
                /* Per-socket SiFive CLINT */
                riscv_aclint_swi_create(
                    memmap[SANCTUM_CLINT].base + i * memmap[SANCTUM_CLINT].size,
                    base_hartid, hart_count, false);
                riscv_aclint_mtimer_create(memmap[SANCTUM_CLINT].base +
                        i * memmap[SANCTUM_CLINT].size + RISCV_ACLINT_SWI_SIZE,
    ...
    if (riscv_is_32bit(&s->soc[0])) {
        ...
        virt_high_pcie_memmap.base = memmap[SANCTUM_DRAM].base + machine->ram_size;
    ...
    /*sanctum: elf loader module */
    memory_region_init_rom(elfld_rom, NULL, "riscv.sanctum.elfldr",
                            memmap[SANCTUM_ELFLD].size, &error_fatal);
                            
    memory_region_add_subregion(system_memory, sanctum_memmap[SANCTUM_ELFLD].base,
                            elfld_rom);
    ...
    gpex_pcie_init(system_memory,
                   memmap[SANCTUM_PCIE_ECAM].base,
                   memmap[SANCTUM_PCIE_ECAM].size,
                   memmap[SANCTUM_PCIE_MMIO].base,
                   memmap[SANCTUM_PCIE_MMIO].size,
                   virt_high_pcie_memmap.base,
                   virt_high_pcie_memmap.size,
                   memmap[SANCTUM_PCIE_PIO].base,
                   DEVICE(pcie_irqchip));
    ...
    sysbus_create_simple("goldfish_rtc", memmap[SANCTUM_RTC].base,
    ...
    /*SANCTUM  CHANGES start*/
    //firmware loading is done with sanctum_virt_machine_done
    //works as reset code see riscv_find_and_load_firmware
    //to load  in mrom see riscv_setup_rom_reset-vec
...
static void sanctum_virt_machine_class_init(ObjectClass *oc, void *data)
{
    char str[128];
    ...
    mc->init = sanctum_virt_machine_init;
    mc->max_cpus = 1 /* VIRT_CPUS_MAX */;
    ...
    mc->default_ram_id = "riscv_sanctum_board.ram";
...
static const TypeInfo virt_machine_typeinfo = {
    ...
    .parent     = TYPE_MACHINE,
    .class_init = sanctum_virt_machine_class_init,
